...
MODEL_SAVE_PATH = "runs/train/exp/weights/best.pt"  # Percorso modello addestrato

# Mappa delle versioni YOLO ai modelli disponibili
YOLO_MODELS = {
    "YOLOv5": ["YOLOv5n", "YOLOv5s", "YOLOv5m", "YOLOv5x6u", "YOLOv5x"],
    "YOLOv6": ["YOLOv6n", "YOLOv6s", "YOLOv6m", "YOLOv6l", "YOLOv6x"],
    "YOLO11": ["YOLO11x", "YOLO11n", "YOLO11s", "YOLO11l"],
}
...
class VideoProcessingThread(QThread):
    ...
    def __init__(self, video_source, detector):
        super().__init__()
    ...
    def run(self):
        ...
        if not cap.isOpened():
            print("Errore: impossibile aprire il video o flusso.")
...
class VideoProcessorApp(QMainWindow):
    def __init__(self):
        ...
        # Variabile di stato per controllare il ciclo di elaborazione
        ...
        self.train_button = QPushButton("Addestra Modello YOLO su COCO8")
        self.train_button.clicked.connect(self.train_yolo_model)
        self.layout.addWidget(self.train_button)

        # Pulsante per elaborare video o live stream
    ...
    def stop_processing(self):
        """Interrompe l'elaborazione del video o dello stream."""
    ...
    def process_video(self):
        ...
        # Avvia il thread di elaborazione video
        self.video_thread = VideoProcessingThread(video_source, self.detector)
    ...
    def update_video_frame(self, qimage):
        """Aggiorna il frame video nell'interfaccia."""
        self.video_label.setPixmap(QPixmap.fromImage(qimage).scaled(
            self.video_label.width(), self.video_label.height(), Qt.KeepAspectRatio
        ))

    def on_processing_finished(self):
        """Azioni da eseguire al termine dell'elaborazione."""
        self.video_label.setText("Elaborazione completata.")
        self.video_thread = None

    def train_yolo_model(self):
        """Addestra un modello YOLO utilizzando il dataset COCO8."""
        self.video_label.setText("Addestramento modello YOLO in corso...")
        self.progress_bar.setValue(0)
        QApplication.processEvents()

        try:
            model = YOLO("yolov8n.pt")  # Utilizza un modello YOLO pre-addestrato
            model.train(data="coco8.yaml", epochs=10, imgsz=640)
            self.video_label.setText("Addestramento completato! Modello salvato.")
            self.progress_bar.setValue(100)
        except Exception as e:
            self.video_label.setText(f"Errore durante l'addestramento: {e}")
